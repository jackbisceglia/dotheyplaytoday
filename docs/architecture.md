...
the view by `emailVerified`, without another lookup or email normalization.
It forwards the generated URL unchanged. A shared Better Auth before hook selects the Web root through `callbackURL`, adding `confirmed=1`

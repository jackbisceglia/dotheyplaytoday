...
export function Form(props: FormProps) {
  ...
  const [activeLeague, setActiveLeague] = createSignal(leagues()[0]?.id ?? "");
  ...
  const [teamMessage, setTeamMessage] = createSignal<TeamMessage>();
  ...
  const toggleTeam = (teamId: string) => {
    ...
    if (!current.has(teamId) && current.size >= subjectCapacity) {
      setTeamMessage({ kind: "hint", text: capacityHint });
      return;
    }
    ...
    setSelected(next);
    setTeamMessage(
      next.size === subjectCapacity
        ? { kind: "hint", text: capacityHint }
        : undefined,
    );
  };
  ...
  const focusFirstInvalidControl = () => {
    ...
    if (selected().size === 0) {
      root
        ?.querySelector<HTMLButtonElement>(".team-grid:not([hidden]) .team-card")
        ?.focus();
    }
  };
  ...
  const submit = (event: SubmitEvent) => {
    ...
    // `Intl` resolves the *viewer's* zone, so this stays inside the submit
    // handler where it only ever runs in the browser.
          ...
          <fieldset class="form-section" aria-describedby="team-message">
            ...
            <div class="team-message-slot">
              <p
                id="team-message"
                class={
                  teamMessage()?.kind === "error" ? "form-error" : "form-hint"
                }
                role={teamMessage()?.kind === "error" ? "alert" : "status"}
                hidden={teamMessage() === undefined}
              >
                {teamMessage()?.text ?? ""}
              </p>
            </div>
